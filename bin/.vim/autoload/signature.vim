...
" HH extension to temporarily disable/re-enable 
" from vim_as_ide.vim script
function! signature#TempDisable() 
 let b:sig_reenable = 0
 if ( b:sig_enabled )
    let b:sig_reenable = 1
    call signature#Toggle()
 endif
endfunction
function! signature#TempReEnable() 
 if ( b:sig_reenable )
    call signature#Toggle()
 endif
endfunction
