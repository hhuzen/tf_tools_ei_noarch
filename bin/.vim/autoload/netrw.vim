...
"
"  Changes: 150->151 
"               - added support for the IBM MVS ftp server for access to 
"                 MVS datasets as well as the HFS (unix) filesystem
"               - restructured NetrwRemoteFtpCmd (with ftp-i/o in the same
"                 buffer) and various other ftp-command-executions (in a
"                 temporary buffer) into a new function NetrwFtpCmd
"               - implemented all ftp flavours in NetrwFtpCmd:
"                   method 2:ftp+netrc
"                   method 3:ftp+mip
"                   method 9:sftp
"               - sftp is now fully functional
"               - syntax file : added MVS datasets support 
"               - help file : added MVS datasets support 
"               - added netrw#ErrorMsgs for multiline error information
" =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
" Load Once: {{{1
...
"======== In next description the :DechoXYZ is spelled :D-echoXYZ to avoid
"======== substitution problems
"Switch debugging on (:D-echoOn without -) or off (:D-echoOff without -). The debug output is sent
"to a window or tab. Depending on the os, may want to use D-echoRemOn (window) 
"or D-echoTabOn  (debug tab) here.
"DechoTabOn     
"D-echoRemOn
...
"   1=warning  = s:WARNING
"   2=error    = s:ERROR
"  Jan 03, 2014 : max errnum currently is 96
fun! netrw#ErrorMsg(level,msg,errnum)
"  call Dfunc("netrw#ErrorMsg(level=".a:level." msg<".a:msg."> errnum=".a:errnum.") g:netrw_use_errorwindow=".g:netrw_use_errorwindow)
  let msgs = []
  call add ( msgs, a:msg )
  call netrw#ErrorMsgs(a:level,msgs,a:errnum)
"  call Dret("netrw#ErrorMsg")
endfun

" ---------------------------------------------------------------------
" netrw#ErrorMsgs: {{{2
"   extended version of netrw#ErrorMsg for more information output
fun! netrw#ErrorMsgs(level,msgs,errnum)
"  call Dfunc("netrw#ErrorMsgs(level=".a:level." errnum=".a:errnum.") g:netrw_use_errorwindow=".g:netrw_use_errorwindow)

  if a:level < g:netrw_errorlvl
"   call Dret("netrw#ErrorMsgs : suppressing level=".a:level." since g:netrw_errorlvl=".g:netrw_errorlvl)
   return
  endif
...
"    call Decho("setl ma noro")
    setl ma noro
    for msg in a:msgs
     keepj call setline(line("$")+1,level.msg)
    endfor
...
"    call Decho("setl ma noro")
    setl ma noro
    for msg in a:msgs
     call setline(line("$")+1,level.msg)
"     call Decho("wrote msg<".level.msg."> to NetrwMessage win#".winnr())
    endfor
    call setline(line("$")+1,'')
   endif
   if &fo !~ '[ta]'
...
"   call Decho("(netrw#ErrorMsgs) setl noma ro bh=wipe")
...
"   redraw!
   ...
   for msg in a:msgs
    ...
    sleep 2
    echomsg level.msg
"    call Decho("echomsg ***netrw*** ".msg)
   endfor
   echohl None
  endif
"  call Dret("netrw#ErrorMsgs")
endfun
...
" values for s:netrw_ftpservertype to specify the type of FTP server
" the IBM FTP server requires some special ftp commands
if ! exists("s:FTPSERVERTYPE_GENERAL")
 call s:NetrwInit("s:FTPSERVERTYPE_GENERAL",0)
 call s:NetrwInit("s:FTPSERVERTYPE_IBM_FTP_MVS",1)
 call s:NetrwInit("s:FTPSERVERTYPE_IBM_FTP_HFS",2)
endif

" values for s:netrw_listtype to specify the type of directory listing
" returned from the remote server
" the IBM FTP server, for access to MVS, produces a special type of listing
if ! exists("s:LISTTYPE_GENERAL")
 call s:NetrwInit("s:LISTTYPE_GENERAL",0)
 call s:NetrwInit("s:LISTTYPE_IBM_FTP_MVS",1)
endif

" liststyles 
"   liststyle toggling applies to unix-like-ls style listings and
"   cycles through values 0 to (MAXLIST -1) 
"   for the IBM FTP server the MVS listing does not allow this toggling
if !exists("s:LONGLIST")
 call s:NetrwInit("s:MINLIST" ,-2)
 call s:NetrwInit("s:MVSLIST" ,-1)
...
call s:NetrwInit("g:netrw_dirhist_0","")

" g:netrw_ignorebanner: strings to ignore in remote server banner output
" the <ESC>?1034h sequence is an annoying terminal output code, seems to be
" readline-related, possibly originating from the local shell for running the client command
call s:NetrwInit("g:netrw_ignorebanner",'^\(\|.*1034h\|Trying .*\|IBM FTP CS.*\|Connected to.*\)$' )

" g:netrw_sftp_errors: strings to flag as errors in sftp output
call s:NetrwInit("g:netrw_sftp_errors",'^\(Can.t ls:\|Couldn.t canonicalise:\|530 .*\)')

" s:netrw_listtype: specify the type of directory listing
" returned from the remote server
" the IBM FTP server, for access to MVS, produces a special type of listing
call s:NetrwInit("s:netrw_listtype",s:LISTTYPE_GENERAL)

" values for s:netrw_ftpservertype to specify the type of FTP server
" the IBM FTP server requires some special ftp commands
call s:NetrwInit("s:netrw_ftpservertype",s:FTPSERVERTYPE_GENERAL)
...
"g:netrw_ftp_browse_reject:  is a pattern match string (for use in a edit-substitute command)
" to remove individual lines from the ftp command output
call s:NetrwInit("g:netrw_ftp_browse_reject", '^total\s\+\d\+$\|^Trying\s\+\d\+.*$\|^KERBEROS_V\d rejected\|^Security extensions not\|No such file\|: connect to address [0-9a-fA-F:]*: No route to host$')
call s:NetrwInit("g:netrw_ftp_ibm_browse_reject", '^total\s\+\d\+$\|^Trying\s\+\d\+.*$\|^KERBEROS_V\d rejected\|^Security extensions not\|No such file\|: connect to address [0-9a-fA-F:]*: No route to host\|^IBM FTP CS.*\|^Connecting to:.*\|^2\d\d\|^125\|^331\|^>>>\|^Command:\|^QUIT generated')

"g:netrw_ftp_lcd_msg:  is a pattern match string (for use in a edit-substitute command)
"  to remove the (language locale dependent) responses to the lcd command
call s:NetrwInit("g:netrw_ftp_lcd_msg", 'Local directory now.*\|Lokale map nu.*')
...
" sanity checks
if g:netrw_liststyle <= s:MINLIST || g:netrw_liststyle >= s:MAXLIST
...
"     call Decho("exe keepj lcd ".fnameescape(curdir))  " NOTE: was g:netrw_fname_escape for some reason
  ...
  try
    ...
    " gives an error if vim is not compiled with clipboard support -- ignore it
  endtry
...
"          dosplit==0: the window will be split if the current file has been modified and hidden not set
...
"  call Dret("netrw#NetrwMakeTgt s:netrwmftgt=".s:netrwmftgt)
endfun
...
"   call Decho("obtain a file from local ".b:netrw_curdir." to ".tgtdir)
   if exists("b:netrw_curdir") && getcwd() != b:netrw_curdir
    let topath= s:ComposePath(tgtdir,"",'f')
    if (has("win32") || has("win95") || has("win64") || has("win16"))
...
"    call Decho("exe ".s:netrw_silentxfer."!".g:netrw_scp_cmd.shellescape(useport,1)." ".filelist." ".shellescape(tgtdir,1))
   ...
   ".........................................
   " NetrwObtain: ftp method #2 (ftp+.netrc) #3 (ftp + mip) or #9 (sftp) {{{3
   elseif b:netrw_method  == 2 || b:netrw_method == 3 || b:netrw_method == 9

    let ftpcmds = []
    call add ( ftpcmds, 'cd "'.b:netrw_fname.'"' )
    for fname in fnamelist
     call add( ftpcmds, 'get "'.fname.'"' )
    endfor
    call s:NetrwFtpCmd( -1, ftpcmds )

   else
    call netrw#ErrorMsg(s:ERROR,"unsupported method",96) 
   endif
...
"    call Dret("netrw#NetRead :3 getcwd<".getcwd().">")
   ...
   " Check whether or not NetrwBrowse() should be handling this request
   " for access to IBM MVS datasets (and target is not a MVS dataset member)
   
   if  s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS && choice !~ "\("
"    call Decho("yes, choice matches IBM MVS dataset qualifier")
    keepj call s:NetrwBrowse(0,choice)
"    call Dret("netrw#NetRead :3 getcwd<".getcwd().">")
    return
   endif
...
"   call Decho("executing: !".g:netrw_rcp_cmd." ".s:netrw_rcpmode." ".shellescape(uid_machine.":".b:netrw_fname,1)." ".shellescape(tmpfile,1))
   ...
   ".........................................
   " NetRead: ftp method #2 (ftp+.netrc) #3 (ftp + mip) or #9 (sftp) {{{3
   elseif b:netrw_method  == 2 || b:netrw_method == 3 || b:netrw_method == 9
    let ftpcmds = []
    let utmpfile= substitute(tmpfile, '\' , '/' , 'ge')
    call add ( ftpcmds, 'get "'.b:netrw_fname.'" "'.utmpfile.'"' )
    call s:NetrwFtpCmd( -1, ftpcmds )
...
"    call Dret("netrw#NetWrite : cannot write on browse window!")
    return
  endif
...
"    call Decho("executing: !".g:netrw_rcp_cmd." ".s:netrw_rcpmode." ".shellescape(tmpfile,1)." ".shellescape(uid_machine.":".b:netrw_fname,1))
   ...
   ".........................................
   " NetWrite: ftp method #2 (ftp+.netrc) #3 (ftp + mip) or #9 (sftp) {{{3
   elseif b:netrw_method  == 2 || b:netrw_method == 3 || b:netrw_method == 9
    let utmpfile= substitute(tmpfile, '\' , '/' , 'ge')
    let ftpcmds = []
    if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS )
       " patch: using vim on ftp://<mvshost>//'AAAAA.BBBB.CCCC(DDD)'  
       " the netrw_fname turned out to be \'AAAAA.BBBB.CCCC(DDD)\'  
       " must get rid of the \  
       let x= b:netrw_fname  
       let x = substitute(x,'\\','','g')  
"       call Decho("fix netrw_fname : ".b:netrw_fname . " -> " . x  )  
       call add(ftpcmds, 'put "'.utmpfile.'" "'.x.'"' )
    else
       call add(ftpcmds, 'put "'.utmpfile.'" "'.b:netrw_fname.'"'  )
    endif
...
"  call Dfunc("NetrwGetFile(readcmd<".a:readcmd.">,tfile<".a:tfile."> method<".a:method.">)")
  ...
  if a:readcmd == 't'
"   call Decho("(NetrwGetFile)  ro=".&l:ro." ma=".&l:ma." mod=".&l:mod." wrap=".&l:wrap." (filename<".expand("%")."> buf#".bufnr("%")." win#".winnr()." ft<".&ft.">)")
...
"   call Decho("sftp://...")
   ...
   let g:netrw_ftp_list_cmd     = "ls -l"
   let g:netrw_ftp_timelist_cmd = "ls -l"
   let g:netrw_ftp_sizelist_cmd = "ls -l"
...
" ---------------------------------------------------------------------
"  s:NetrwFtpSetServerType
"
"  if s:machine matches (comma separated) netrw_ftp_ibm_server_list
"  then initialize for use of IBM server accessing either
"  - the hfs filesystem (like unix)
"  - mvs datasets
"
fun s:NetrwFtpSetServerType()

   let s:netrw_ftpservertype = s:FTPSERVERTYPE_GENERAL 
   let s:netrw_listtype = s:LISTTYPE_GENERAL 

   if exists("g:netrw_ftp_ibm_server_list")
	if ( ",".g:netrw_ftp_ibm_server_list."," =~ ".*,".g:netrw_machine.",.*" )
     		if b:netrw_fname =~ ".*'.*" 
 			let s:netrw_ftpservertype = s:FTPSERVERTYPE_IBM_FTP_MVS 
 			let s:netrw_listtype = s:LISTTYPE_IBM_FTP_MVS 
"   			call Decho("(NetrwFtpSetServerType) set FTPSERVERTYPE_IBM_FTP_MVS: ". g:netrw_machine . " " . b:netrw_fname )
  			let g:netrw_liststyle = s:MVSLIST
			" also override w:netrw_liststyle
  			let w:netrw_liststyle = s:MVSLIST

   			let g:netrw_sort_direction = ''
   			let g:netrw_sort_by = ''
   			if !exists("g:netrw_ftp_ibm_mute_regexp")
				" default is : there should be at least 2 name
				" qualifiers
				call s:NetrwInit("g:netrw_ftp_ibm_mute_regexp", "^[^\\.]*[\\.]*$")
			endif
		else    " when switching between HFS and MVS, better reinitialize sort spec
 			let s:netrw_ftpservertype = s:FTPSERVERTYPE_IBM_FTP_HFS 
 			" s:netrw_listtype remains GENERAL
  			let g:netrw_liststyle = s:THINLIST
  			let w:netrw_liststyle = s:THINLIST
   			let g:netrw_sort_by = 'name'
   			let g:netrw_sort_direction = 'normal'
"   			call Decho("(NetrwFtpSetServerType) set FTPSERVERTYPE_IBM_FTP_HFS:". g:netrw_machine . " " . b:netrw_fname )
		endif
 		let g:netrw_ftp_list_cmd     = "dir"
 		let g:netrw_ftp_timelist_cmd = "dir"
 		let g:netrw_ftp_sizelist_cmd = "dir"
...
"   call Decho("(NetrwMaps) make local maps")
 ...
 "  nnoremap <buffer> <silent> gb	:call <SID>NetrwBookHistHandler(1,b:netrw_curdir)<cr>
   ...
   nnoremap <buffer> <silent> qb	:call <SID>NetrwBookHistHandler(2,b:netrw_curdir)<cr>
   ...
   inoremap <buffer> <silent> gb	<c-o>:call <SID>NetrwBookHistHandler(1,b:netrw_curdir)<cr>
   "inoremap <buffer> <silent> gb	:call <SID>NetrwBookHistHandler(1,b:netrw_curdir)<cr>
   ...
   inoremap <buffer> <silent> qb	:call <SID>NetrwBookHistHandler(2,b:netrw_curdir)<cr>
   ...
   nnoremap <buffer> <F5> :call <SID>NetrwBookHistHandler(2,b:netrw_curdir)<cr>
   nnoremap <buffer> <F10> :call <SID>NetrwModeToggle(b:netrw_curdir)<cr>
...
"    nno  <buffer> <silent>	<Plug>NetrwLeftdrag	:call <SID>NetrwLeftdrag(0)<cr>
   ...
   inoremap <buffer> <F5>                       <c-o>:call <SID>NetrwBookHistHandler(2,b:netrw_curdir)<cr>
   inoremap <buffer> <F10>                      :call <SID>NetrwModeToggle(b:netrw_curdir)<cr>
  endif
...
" ---------------------------------------------------------------------
"  s:NetrwModeToggle 
"  based on user customization g:netrw_ftp_enc_toggle
"  prepares encoding handling for ftp data
let g:netrw_ftp_enc_toggle_manual=0
let g:netrw_ftp_enc_toggle_index=0
let g:netrw_ftp_enc_toggle_cp='UTF-8'
let g:netrw_ftp_enc_toggle_mode='binary'
let g:netrw_ftp_enc_toggle_extracmd=''
"
fun! s:NetrwModeToggle(curdir)
"  call Dfunc("s:NetrwModeToggle()")

  if !exists( "g:netrw_ftp_enc_toggle" ) 
"   call Dret("s:NetrwModeToggle no config")
   return
  endif
  let g:netrw_ftp_enc_toggle_manual=1

"  call Decho("HH1 max ".len(g:netrw_ftp_enc_toggle))


  let g:netrw_ftp_enc_toggle_index = g:netrw_ftp_enc_toggle_index + 1
  if ( g:netrw_ftp_enc_toggle_index >= len(g:netrw_ftp_enc_toggle) ) 
     let g:netrw_ftp_enc_toggle_index = 0
  endif
  let e=get(g:netrw_ftp_enc_toggle, g:netrw_ftp_enc_toggle_index) 
  let g:netrw_ftp_enc_toggle_cp = get( e, 0) 
  let g:netrw_ftp_enc_toggle_mode = get( e, 1) 
  let g:netrw_ftp_enc_toggle_extracmd = get( e, 2) 

  let g:netrw_ftpmode =g:netrw_ftp_enc_toggle_mode
"  call Decho("s:NetrwModeToggle x ".g:netrw_ftp_enc_toggle_index )
"  call Decho("s:NetrwModeToggle cp ". g:netrw_ftp_enc_toggle_cp)
"  call Decho("s:NetrwModeToggle mode ". g:netrw_ftp_enc_toggle_mode)
"  call Decho("s:NetrwModeToggle extra ". g:netrw_ftp_enc_toggle_extracmd)

  let svpos= netrw#NetrwSavePosn()
  if ( a:curdir =~ 'ftp:.*' ) 
     keepj call s:NetrwRefresh(0,a:curdir)
  else
     keepj call s:NetrwRefresh(1,a:curdir)
  endif
  keepj call netrw#NetrwRestorePosn(svpos)
"  call Dret("s:NetrwModeToggle")
  "echo "         "
endfun

" ---------------------------------------------------------------------
"  s:NetrwModeAutoSelect 
"  As long as there has not been a manual intervention, select mode
"  based on path.
fun! s:NetrwModeAutoSelect()
"  call Dfunc("s:NetrwModeAutoSelect()")

  if !exists( "g:netrw_ftp_enc_toggle" ) 
"   call Dret("s:NetrwModeAutoSelect no config")
   return
  endif
  if ( g:netrw_ftp_enc_toggle_manual == 1 ) 
"   call Dret("s:NetrwModeAutoSelect manual")
   return
  endif
  let cp='default'
  if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS )
  ...
  if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_HFS )
    let cp='IBM-1047'
  endif
  let x=0
  while ( x < len(g:netrw_ftp_enc_toggle) ) 
    let e=get(g:netrw_ftp_enc_toggle, x) 
    if ( get(e,0) =~ '.*'.cp.'.*' )
      let g:netrw_ftp_enc_toggle_index = x
      let g:netrw_ftp_enc_toggle_cp = get( e, 0) 
      let g:netrw_ftp_enc_toggle_mode = get( e, 1) 
      let g:netrw_ftp_enc_toggle_extracmd = get( e, 2) 
      break
  ...
  let g:netrw_ftpmode =g:netrw_ftp_enc_toggle_mode
"  call Decho("s:NetrwModeAutoSelect x ".g:netrw_ftp_enc_toggle_index )
"  call Decho("s:NetrwModeAutoSelect encoding". g:netrw_ftp_enc_toggle_cp)
"  call Decho("s:NetrwModeAutoSelect mode ". g:netrw_ftp_enc_toggle_mode)
"  call Decho("s:NetrwModeAutoSelect extra ". g:netrw_ftp_enc_toggle_extracmd)
"  call Dret("s:NetrwModeAutoSelect")
  return
endfun
...
"     call Decho("Netrw Bookmark#".cnt.": ".g:netrw_bookmarklist[cnt-1])
     echo printf("%2d Bookmark#%-2d: %s (%d)",qcnt ,cnt,g:netrw_bookmarklist[cnt-1], qcnt)
     let didwork = 1
     call add(qdir, g:netrw_bookmarklist[cnt-1])
     let qcnt     = qcnt + 1
     let cnt     = cnt + 1
    endfor
   endif
...
"     call Decho("Netrw  History#".histcnt.": ".g:netrw_dirhist_{cnt})
      echo printf("%2d  History#%-2d: %s (%d)",qcnt, histcnt,g:netrw_dirhist_{cnt}, qcnt)
      let didwork= 1
     call add(qdir, g:netrw_dirhist_{cnt})
      let qcnt     = qcnt + 1
     endif
   ...
   "last remote (index=qcnt)
   echo printf("<F5>  LastFTP : %s <F5>", g:netrw_dirhist_0)
   call add(qdir, g:netrw_dirhist_0)

   if didwork
    call inputsave()
    let chinput=input("Press <cr> to continue or <nr><cr> or enter ftp:/.... to change dir : ")
    call inputrestore()
    if ( chinput =~ "ftp:" ) 
      " use input as new place to go
      let qdir[qcnt] = chinput       
      let chinput = qcnt 
    elseif ( chinput =~ "<F5>" ) 
      let chinput = qcnt 
    endif
    if ( chinput != "" ) 
       if exists("qdir[chinput]")
        exe "keepj e ".fnameescape( qdir[chinput] )
       else
        echomsg " - Sorry, #".chinput." doesn't exist!"
       endif
    endif
   endif
   let g:netrw_ftp_enc_toggle_manual =0
  ...
  elseif a:chg == 3
   " saves most recently visited directories (when they differ) - and when not yet in history
   let qcnt = 1
   let matched = 0
   while (qcnt <= g:netrw_dirhistmax)
      if ( exists("g:netrw_dirhist_{qcnt}") && g:netrw_dirhist_{qcnt} == a:curdir ) 
        let matched = 1
      endif
      let qcnt = qcnt + 1
   endwhile
...
"     call Decho("HH1")
   if ( a:curdir =~ 'ftp:.*' ) 
     let g:netrw_dirhist_0 = a:curdir
"     call Decho("HH2")
   endif
...
"   call Dret("s:NetrwBookHistSave : dirhistmax=".g:netrw_dirhistmax)
  ...
  " 0 = last remote
  let x=g:netrw_dirhist_0
  let x=substitute( x, "'", "''", "g") 
  call setline(3,"let g:netrw_dirhist_0='".x."'")

  let lastline = line("$")
  let cnt      = 1
  while cnt <= g:netrw_dirhist_cnt
   let x=g:netrw_dirhist_{cnt}

   " MVS dataset names may have ' quote, must turn info ''
   " fnameescape turns it into \' but that gives a syntax error
   let x=substitute( x, "'", "''", "g") 
   call setline((cnt+lastline),'let g:netrw_dirhist_'.cnt."='".x."'")
   let cnt= cnt + 1
  endwhile
...
"   call Decho("(NetrwBrowse) clearing marked files")
  ...
  let is_mvs_dataset=0
  if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS )
    let is_mvs_dataset=1
"  call Decho("(NetrwBrowse) x is_mvs_dataset=".is_mvs_dataset)
    if ( dirname =~ "\(" ) 
      let is_mvs_dataset=0
"  call Decho("(NetrwBrowse) x is_mvs_dataset=".is_mvs_dataset)
    endif
...
"  call Decho("(NetrwBrowse) dirname=".dirname)
"  call Decho("(NetrwBrowse) s:netrw_ftpservertype=".s:netrw_ftpservertype)
"  call Decho("(NetrwBrowse) s:netrw_listtype=".s:netrw_listtype)
"  call Decho("(NetrwBrowse) is_mvs_dataset=".is_mvs_dataset)
...
"   call Decho("(NetrwBrowse) getcwd<".getcwd().">")
  ...
  elseif !a:islocal && dirname !~ '[\/]$' && dirname !~ '^"' && is_mvs_dataset == 0 
...
"  call Dfunc("s:NetrwGetBuffer(islocal=".a:islocal." dirname<".a:dirname.">) g:netrw_liststyle=".g:netrw_liststyle . "  w:netrw_liststyle=".  w:netrw_liststyle )
...
"   call Decho("(NetrwGetBuffer) case listtyle not treelist: find buffer number of buffer named precisely the same as dirname--")
...
"   call Decho("obtain word from wide listing")
   ...
   if ( s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS )
"   	call Decho("selection for MVS in: " . dirname)
	if dirname =~ "^ "              " banner line
     		let dirname= "./"
	elseif dirname =~ "'$"		" dataset
   		let dirname= substitute(dirname,"^[^']*",'','')
	else				" dataset member
   		let dirname= substitute(dirname," .*",'','')
	endif
"   	call Decho("selection for MVS out: " . dirname)
   endif

   if ( s:netrw_listtype == s:LISTTYPE_GENERAL )
...
"  call Dfunc("NetrwListStyle(islocal=".a:islocal.") w:netrw_liststyle=".w:netrw_liststyle)
  ...
  if ( w:netrw_liststyle == s:MVSLIST ) 	" no cycling when using MVSLIST
	  return
  endif
...
"  call Decho("(NetrwBrowseChgDir) dirname<".dirname.">  dirpat<".dirpat."> newdir <".newdir .">")
...
"    call Decho("(NetrwBrowseChgDir) adjusting dirname<".dirname.">")
  ...
  let target_is_a_file = 0 
  if ( s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS )
	if ( newdir =~ "'" || newdir =~ "/" )
  		let target_is_a_file = 0 
	else
  		let target_is_a_file = 1 
	endif
"        call Decho("(NetrwBrowseChgDir) MVS: target_is_a_file =".target_is_a_file)
  elseif newdir !~ dirpat 
  	let target_is_a_file = 1 
  endif

  if target_is_a_file == 1
   " ------------------------------
...
"   call Decho("(NetrwBrowseChgDir:edit-a-file) win#".winnr()." buf#".bufnr("%")."<".bufname("%")."> ft=".&ft)
   ...
   if ( s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS )
        let dirname= substitute(dirname,"'$","(".newdir.")'",'')
"    	call Decho("(NetrwBrowseChgDir:edit-a-MVSdatasetmember) " . dirname )
   else
...
"     call Decho("(NetrwBrowseChgDir:edit-a-file) tree listing")
    elseif newdir =~ '^\(/\|\a:\)'
     ...
     let dirname= s:ComposePath(dirname,newdir,'d')
    endif
   endif
...
"    call Decho("(NetrwBrowseChgDir:go-up) setl noro ma")
   ...
   if s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS 
        let dirname= substitute(dirname,"\.[^\.]*$","'",'')
"        call Decho("(NetrwBrowseChgDir:go-up one MVSdataset qualifier level) " . dirname )
   elseif has("amiga")
    " amiga
...
"    call Decho("(NetrwBrowseChgDir:tree-list) go down one dir: treedir<".treedir.">")
   ...
   if s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS 
        let dirname= substitute(dirname,"'.*'",newdir,'')
        let dirname= substitute(dirname,"/$",'','')
"        call Decho("(NetrwBrowseChgDir:go-down a MVSdataset lowerlevel) " . dirname )
   else
     let dirname    = s:ComposePath(dirname,newdir,'d')
...
"  call Decho("set up redirection: redir{".redir."} srr{".&srr."}")
  ...
  " extract any viewing options.  Assumes that they're set apart by list.
...
"  call Dfunc("NetrwMakeDir(usrhost<".a:usrhost.">)")
  ...
  " a message.
  call inputsave()
  let newdirname= input("Enter directory name: ")
  call inputrestore()
...
"    call Decho("exe sil! !".mkdircmd." ".shellescape(newdirname,1))
 ...
 "   redraw!
   ...
   elseif b:netrw_method == 2 || b:netrw_method == 3 || b:netrw_method == 9
    let ftpcmds = []
    if ( s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS )
    	let x = substitute(b:netrw_curdir,"^[^']*","","")
    	let newdirname = substitute(x,"'$",".".newdirname."'","")
    else
	    " must do a cd to the current directory else relative
	    " directory name is not handled correctly
    	let prefix=substitute(b:netrw_curdir, b:netrw_fname.'.*', '', '')
	let dirpath=substitute(b:netrw_curdir, "^".prefix, '', '')
    	call add ( ftpcmds, 'cd "'.dirpath.'"')
    endif

    let svpos= netrw#NetrwSavePosn()
    call add ( ftpcmds, g:netrw_remote_mkdir.' "'.newdirname.'"')
    call s:NetrwFtpCmd( -1, ftpcmds )
    keepj call s:NetrwRefresh(0,s:NetrwBrowseChgDir(0,'./'))
    keepj call netrw#NetrwRestorePosn(svpos)
   else
...
"   call Decho("ending s:netrwmarkfilemtch_".curbufnr."<".s:netrwmarkfilemtch_{curbufnr}.">")
  ...
  if exists("s:netrwmarkfilelist")
   let dname= s:ComposePath(b:netrw_curdir,a:fname,'f')
   if index(s:netrwmarkfilelist,dname) == -1
    " append new filename to global markfilelist
    call add(s:netrwmarkfilelist,s:ComposePath(b:netrw_curdir,a:fname,'f'))
...
"     call Decho("fname<".fname."> is compressed so decompress with <".exe.">")
     ...
     if a:islocal
      if g:netrw_keepdir
       let fname= shellescape(s:ComposePath(curdir,fname,'f'))
      endif
     else
    ...
    elseif a:islocal
     ...
     call system(netrw#WinPath(g:netrw_compress)." ".shellescape(s:ComposePath(b:netrw_curdir,fname,'f')))
    else
     " fname not a compressed file, so compress it
...
"   call Decho("copy from remote to remote")
   ...
   " NB: using tmpfilename as tempdir name
   if tmpdir !~ '[/\\]'			" should look like a unix or windows path
    let tmpdir= curdir."/".tmpdir
   endif
   if exists("*mkdir")
"    call Decho("mkdir ". tmpdir)
    call mkdir(tmpdir)
   else
"    call Decho("g:netrw_localmkdir ". tmpdir)
...
"    call Decho("HH mc src " .  b:netrw_curdir)
"    call Decho("HH mc tgt " .  s:netrwmftgt)
    if ( b:netrw_curdir == s:netrwmftgt )
     let newlocalfiles = []
     for oldname in localfiles 
       call inputsave()
       let newname= input("Copy ".oldname." to : ",oldname,"file")
       call inputrestore()
       if newname == ""
...
"        call Dret("s:NetrwMarkFileCopy 0 : failed: system(".g:netrw_localcopycmd." ".args." ".shellescape(s:netrwmftgt))
       ...
       call add(newlocalfiles, newname)
     endfor
     let localfiles = newlocalfiles
    endif
    ...
    if getcwd() == tmpdir
     ...
     if ( len(newlocalfiles) > 0 )
       for fname in newlocalfiles
         keepj call s:NetrwDelete(fname)
       endfor
     endif
     exe "keepj lcd ".fnameescape(curdir)
...
"    "   call Dret("s:NetrwMarkFileExe : early exit, empty command")
   ...
   for fname in s:netrwmarkfilelist_{curbufnr}
    if a:islocal
     if g:netrw_keepdir
      let fname= shellescape(netrw#WinPath(s:ComposePath(curdir,fname,'f')))
     endif
    else
...
"  call Decho("sanity chk passed: s:netrwmarkfilelist_".curbufnr."<".string(s:netrwmarkfilelist_{curbufnr}))
  if exists("s:netrwmarkfilelist_{curbufnr}")
   ...
   for fname in netrwmarkfilelist
    if a:islocal
     if g:netrw_keepdir
      let fname= s:ComposePath(curdir,fname,'f')
     endif
    else
...
"   call Decho("handle local regexp")
   ...
   let files   = s:Myglob(s:ComposePath(dirname,regexp,'r'))
...
"    call Decho(" get word <".curword.">")
   let tgtdir = s:ComposePath(curdir,curword,'d')
   if a:islocal && isdirectory(tgtdir)
    let s:netrwmftgt = tgtdir
"    call Decho("local isdir: s:netrwmftgt<".s:netrwmftgt.">")

   elseif ( !a:islocal && s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS )
    if curword =~ '\.'
      let s:netrwmftgt = tgtdir
    else
      let s:netrwmftgt = curdir
    endif
"    call Decho("remote mvs isdir: s:netrwmftgt<".s:netrwmftgt.">")
...
"  call Dfunc("s:NetrwOpenFile(islocal=".a:islocal.")")
  ...
  if fname == ""
   let @@= ykeep
"   call Dret("s:NetrwOpenFile : user aborted with bare <cr>")
   return
  endif

  if ( s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS )
   if fname !~ "'" 
    	let fname = substitute(b:netrw_curdir,"'$","(".fname.")'","")
   endif
   let x= fnameescape(fname)
"   Decho ("new MVS datasetmember: ". x)
   exe "e ".x
  else
   if fname !~ '[/\\]'
    if exists("b:netrw_curdir")
...
"  call Dfunc("s:NetrwUnmarkList(curbufnr=".a:curbufnr." curdir<".a:curdir.">)")
  ...
  if exists("s:netrwmarkfilelist_{a:curbufnr}")
   for mfile in s:netrwmarkfilelist_{a:curbufnr}
    let dfile = s:ComposePath(a:curdir,mfile,'f')       " prepend directory to mfile
...
"    call Decho("done writing local->remote")
  ...
  elseif type(a:fname) == 3
   ...
   if b:netrw_method == 4
"    call Decho("handle uploading a list of files via scp method#4")
...
"    call Decho("exe ".s:netrw_silentxfer."!".g:netrw_scp_cmd.shellescape(useport,1)." ".args." ".shellescape(machine.":".tgt,1))
   ...
   ".........................................
   " NetrwUpload: ftp method #2 (ftp+.netrc) #3 (ftp + mip) or #9 (sftp) {{{3
   elseif b:netrw_method  == 2 || b:netrw_method == 3 || b:netrw_method == 9
     ...
     let ftpcmds = []
     let ufromdir= substitute(fromdir, '\' , '/' , 'ge')
     call add ( ftpcmds, 'lcd "'.ufromdir.'"' )
     call add ( ftpcmds, 'cd "'.tgtdir.'"' )
     for fname in a:fname
      call add ( ftpcmds, 'put "'.fname.'"')
     endfor
    call s:NetrwFtpCmd( -1, ftpcmds )
   else
     call netrw#ErrorMsg(s:ERROR,"unsupported method",96) 
   ...
   endif  " if b:netrw_method
  endif  " if type(a:fname) 
...
fun! s:NetrwSetSort()
"  call Dfunc("SetSort() bannercnt=".w:netrw_bannercnt . " w:netrw_liststyle = " . w:netrw_liststyle)
  if w:netrw_liststyle == s:MVSLIST	"no sort variations for MVS
"  	call Dret("SetSort no setsort for MVS")
	return
  endif
  let ykeep= @@
...
"  call Dfunc("s:NetrwSortStyle(islocal=".a:islocal.") netrw_sort_by<".g:netrw_sort_by.">")
  if w:netrw_liststyle == s:MVSLIST
"  	call Dret("s:NetrwSortStyle : no sort change for MVS")
	return
  endif
...
"   call Decho("(s:PerformListing) set up banner")
   call s:NetrwModeAutoSelect()

   let x=''
   if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_HFS || s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS ) 
    if exists( "g:netrw_ftp_enc_toggle" ) 
     let x = '<F10>:'. g:netrw_ftp_enc_toggle_mode .":". g:netrw_ftp_enc_toggle_cp
    endif
   endif
   let x= x.'                                 '
   let x= strpart ( x, 0, 32 )

   keepj call setline(1,'" ============================================================================')
   keepj call setline(2,'" Netrw Directory Listing     '.x.'    (netrw '.g:loaded_netrw.')')
...
"   call Decho("b:netrw_method=".b:netrw_method)
   if !executable("ftp")
    if !exists("g:netrw_quiet")
     if !executable("ftp")
      keepj call netrw#ErrorMsg(s:ERROR,"'ftp' is not executable on your system",47)
     else
...
"   call Decho("listcmd<".listcmd."> (using g:netrw_ftp_list_cmd)")
   let ftpcmds = []
   call add ( ftpcmds, 'cd "'.s:path.'"')
   if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS )
	   " for MVS dataset filesystem, the regular list command shows either
	   " - the memberlist if the target is a valid partitioned dataset
	   " - the lowerlevel-names in the namespace if the target is not a
	   "   valie partitioned dataset
	   " in the first case, to get lowerlevel info, add 
	   " a "dir 'AAA.BBB.CCC.*'" command to view those lower level datasets
   	   call add ( ftpcmds, listcmd )
    	   let x = substitute(s:path,"'$", ".*'",'')
   	   call add ( ftpcmds, "dir ".x)
   else
   	   call add ( ftpcmds, listcmd.' "'.s:path.'"' )
   endif

   " for IBM MVS datasets, avoid requesting a listing at a high level in the 
   " namespace hierarchy, since that would take minutes or hours 
   " so do some builtin protection : 
   "  - never ask for toplevel ''
   "  - be selective via netrw_ftp_ibm_mute_list (regexp), default: at least 2
   "  qualifiers
   "
   let mute_listing = 0
   if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS )
	let x=substitute( s:path, "'", "", "g" )
	if x == '' || x =~ g:netrw_ftp_ibm_mute_regexp	
		let mute_listing = 1
	endif
   endif
   if mute_listing == 0 
	call s:NetrwFtpCmd( w:netrw_bannercnt, ftpcmds )
   else 	   
        exe "sil! keepalt keepj ".w:netrw_bannercnt
        sil! keepj put=' Muted listing for '.s:path
        exe "sil! keepalt keepj ".w:netrw_bannercnt
   endif
  
   if line("$") > 100 
"   	exe "sil! keepalt keepj ".w:netrw_bannercnt.',100g/^./call Decho("raw Ftp-listing (truncated): ".getline("."))'
"   	Decho("raw listing (truncated): max line = ".line("$"))
   else
"	   exe "sil! keepalt keepj ".w:netrw_bannercnt.',$g/^./call Decho("raw Ftp-listing: ".getline("."))'
   endif

   if ( s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_HFS || s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS ) 
"      call Decho("cleanup for FTPSERVERTYPE_IBM_FTP_HFS")
      "exe 'sil! keepalt keepj /^IBM FTP CS/,/^125/d'
      "exe 'sil! keepalt keepj /^250 List completed/,$d'
      sil! keepj %s/\r$//e
      if g:netrw_ftp_ibm_browse_reject != ""
         exe "sil! keepalt keepj g/".g:netrw_ftp_ibm_browse_reject."/keepj d"
         keepj call histdel("/",-1)
      endif
      keepj call histdel("/",-1)
   endif

   "The netrw_xxx_browse_reject mechanism removes individual lines
   "and may not cleanup command echoing by the server
   "Here a cleanup command is done upto the echoed list command
   if s:method == "sftp" 
      exe 'sil! keepj '.w:netrw_bannercnt.',/^sftp> '.listcmd.'/d'
      keepj call histdel("/",-1)
   endif
   if s:method == "ftp" 
      exe 'sil! keepj '.w:netrw_bannercnt.',/^'.listcmd.'/d'
      keepj call histdel("/",-1)
   endif

   exe "sil! keepalt keepj ".w:netrw_bannercnt
   if ( s:netrw_listtype == s:LISTTYPE_IBM_FTP_MVS )
"      call Decho("do LISTTYPE_IBM_FTP_MVS")

      " for MVS datasets, remove the additional dir command
      exe "sil! keepalt keepj g/^dir .*$/keepj d"
      keepj call histdel("/",-1)
        " make banner lines start with space
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/^.*No members found/ No members found/'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/^.*No datasets found/ No datasets found/'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/^Volume Unit/ Volum Unit/'
      keepj call histdel("/",-1)

      " delete Volum/Unit, preserve Migrated
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/^ Volum Unit   //'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/Dsorg/Do/'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/^T..... ....   //'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/^Migrated                         /Migrated/'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/Ext Used //'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/\([0123456789/]\{10}\)\([0123456789 ]\{9}\)/\1/'
      keepj call histdel("/",-1)
      

      " If the target is not a dataset, then there are no members, and the
      " first dir command shows the lowerlevel datasets are in relative format
      " This output is a duplication of the output of the second dir command.
      " Remove the duplication, change all to absolute format
     
      let sublevels = '' 
      let homedir=b:netrw_curdir
      let homedir = substitute( homedir, "^[^']*'", "", "" )
      let homedir = substitute( homedir, "'.*$", "", "" )
      let seen_dsname_banner=0
      let lastline = line("$")
      let i = w:netrw_bannercnt
      while i <= lastline 
	 let x=getline(i)
	 if seen_dsname_banner == 0 
		if x =~ " Dsname"
		       let seen_dsname_banner = 1
"                       call Decho("got Dsname")
	        endif
         elseif	seen_dsname_banner == 1 
		if x =~ " Dsname"
"                       call Decho("got DsName duplication")
			" hit the duplication, erase it till end of buffer
      			exe 'sil! keepj '.i.',$d'
      			keepj call histdel("/",-1)
			let i = lastline
		elseif x !~ "'"
"                       call Decho("process")
      			exe 'sil! keepj '.i."s/  PO  */  PO  '".homedir."./g"
      			exe 'sil! keepj '.i."s/  PS  */  PS  '".homedir."./g"
      			exe 'sil! keepj '.i."s/ VSAM */ VSAM '".homedir."./g"
      			keepj call histdel("/",-1)
      			exe 'sil! keepj '.i."s/$/'/g"
      			keepj call histdel("/",-1)
		endif
      ...
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/  PS  / PS /'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/  PO  / PO /'
      keepj call histdel("/",-1)
      exe 'sil! keepj '.w:netrw_bannercnt.',$s/ VSAM / VM /'
      keepj call histdel("/",-1)

      " the list of lowerlevel-names may be very long. prune it
      " remove all deeper-than-1 sublevels, but mind that in the namespace
      " there may be qualifiers that are only used in names of lowerlevels
      " like 
      "  'AAA.BBB.CCC" is a dataset
      "  'AAA.BBB.CCC.DDD.EEE" is a dataset
      "  'AAA.BBB.CCC.DDD" itself is not a dataset and is not in the ftp
      "  output
      "
      let sublist=""
      let i = w:netrw_bannercnt
"      call Decho(" HH work homedir ". homedir)
      while i <= lastline 
	 let x=getline(i)
"      call Decho(" HH work on line ".i." : ". x)
	 if  x =~ "'"  
      		let x = substitute( x, "^[^']*'", "", "" )
      		let x = substitute( x, "'.*$", "", "" )
"      call Decho(" HH work on line got x ". x)
		if x != homedir
			let sublist = sublist. ",".x
		endif
 	endif
  	let i = i + 1	
      endwhile
      for sub in split( sublist, "," )
	 if sub != ''
   		exe "sil! keepalt keepj g/'".sub."\.[^']*'$/d"
         	keepj call histdel("/",-1)
	 endif
      endfor

      " put ../ in at front to go up 1 directory, but not if that would be banned by 
      " netrw_ftp_ibm_mute_regexp
      let mute_parent = mute_listing
      let x=substitute( s:path, "'", "", "g" )
      let x=substitute( x, "\.[^\.]*$","", "")
"      Decho("test mute parent for ". x)
      if x == '' || x =~ g:netrw_ftp_ibm_mute_regexp	
		let mute_parent = 1
      endif
"                Decho("test mute_parent= ". mute_parent)
      if mute_parent == 0
           exe "sil! keepalt keepj ".w:netrw_bannercnt
           keepj call histdel("/",-1)
           let x = w:netrw_bannercnt - 1
           exe "sil! keepj ".x
           keepj call histdel("/",-1)
           sil! keepj put='../'
      endif
   ...
   if ( s:netrw_listtype == s:LISTTYPE_GENERAL )
"      call Decho("do LISTTYPE_GENERAL")
	"for navigation, directories must be presented with a trailing /
	"if not yet there
	"  add / for directories and @ for symbbolic links
        sil! keepj g/^d[-rwx][-rwx][-rwx].*[^/]$/keepj s+$+/+e
        sil! keepj g/^l[-rwx][-rwx][-rwx].*[^@]$/keepj s+$+@+e
        keepj call histdel("/",-1)
        keepj call histdel("/",-1)
   endif
   " make sure to point to begin of the list
   exe "sil! keepalt keepj ".w:netrw_bannercnt
...
"      call Decho("search(".'\.\.\/\%(\s\|$\)'."','cnW')=".line2."  w:netrw_bannercnt=".w:netrw_bannercnt)
      if line2 == 0
"       call Decho("netrw is putting ../ into listing (1)")
       sil! keepj put='../'
      endif
      exe "sil! keepj ".line1
      sil! keepj norm! 0
...
"   call Decho("normal ftp cleanup end")
...
"   call Decho("fix long listing: method=" .s:method)
...
"      call Decho("search(".'\.\.\/\%(\s\|$\)'."','cnW')=".line2."  w:netrw_bannercnt=".w:netrw_bannercnt)
      if line2 == 0
"       call Decho("netrw is putting ../ into listing (2)")
       sil! keepj put='../'
      endif
      exe "sil! keepj ".line1
      sil! keepj norm! 0
...
"   exe "keepj ".w:netrw_bannercnt.',$g/^./call Decho("Ftp-listing: ".getline("."))'
"  endif " Decho
"   call Decho("HOI HH1end")
"  call Dret("s:NetrwRemoteListing")
endfun
...
"    call Decho("case all=".all." or ok<".ok.">".(exists("w:netrw_method")? ': netrw_method='.w:netrw_method : ""))
    if exists("w:netrw_method") && (w:netrw_method == 2 || w:netrw_method == 3 || w:netrw_method == 9)
"     call Decho("case ftp:")
     let ftpcmds = []
     ...
     if  s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS 
      if rmfile !~ "'" 
	      let rmfile = substitute(path, "'$" , "(".rmfile.")'", "")
      endif
     else
      if path =~ '^\a\+://'
       let path= substitute(path,'^\a\+://[^/]\+/','','')
      endif
      call add ( ftpcmds, 'cd "'.path.'"')
     endif
     if (w:netrw_method == 9)
      call add ( ftpcmds, "rm ".rmfile)
     else
      call add ( ftpcmds, "delete ".rmfile)
     endif
     call s:NetrwFtpCmd( -1, ftpcmds )
...
"    call Decho("attempt to remove directory")
   ...
   if all || ok =~ 'y\%[es]' || ok == ""
    if exists("w:netrw_method") && (w:netrw_method == 2 || w:netrw_method == 3 || w:netrw_method == 9)
     ...
     call add ( ftpcmds, "rmdir ".a:rmfile)
     call s:NetrwFtpCmd( -1, ftpcmds )
    else
...
" ---------------------------------------------------------------------
"  s:NetrwFtpCmd : execute a set of ftp commands
"  if curbuf_line is -1, then the current buffer is not used, instead a 
"  temporary buffer is used for ftp input/output
"  if curbuf_line is >= 0 then the current buffer is used but for ftp
"  input/output, starting at the indicated line
"
fun s:NetrwFtpCmd( curbuf_line, ftpcmds )
"   call Dfunc("NetrwFtpCmd(curbuf_line<".a:curbuf_line.">")
"   call Decho("NetrwFtpCmd(netrw_method<".b:netrw_method."> path=<".b:netrw_fname.">")

   let netrw_fname = b:netrw_fname 
   let netrw_method = b:netrw_method 

   if (a:curbuf_line < 0 ) 
    "setup a private buffer to use for ftp input and ftp output
    "
    " formerly the "new" cronstruct was used, which
    " caused a new window to be created via NetrwInsureWinVars, 
    " which makes it all more complex 
    " replaced it with the enew construct from netrw(150) NetWrite method#2 
    let bhkeep      = &l:bh
    let curbuf      = bufnr("%")
"    call Decho( "before enew: bufnr(%)=".bufnr("%") )
    setl bh=hide
    keepalt enew
    exe "sil! keepalt file tmpbuf-NetrwFtpCmd"
    setl ff=unix
   else
    " use current buffer, start at line curbuf_line
    let lastline = line("$")
    if (lastline >= a:curbuf_line)
      exe a:curbuf_line.",$d"
    endif
   endif
"   call Decho( "bufnr(%)=".bufnr("%") )

   " pass netrw_fname (full directory path) to NetrwFtpPrepareCmd in stead of
   " fname[0], which may be a basefilename
   let ftp_command_line = s:NetrwFtpPrepareCmd( netrw_method, netrw_fname )

   " ftpcmds may be a \n separated list
   for cmd in a:ftpcmds
     keepj call setline(line("$")+1,cmd)
   endfor

   call s:NetrwFtpExecuteCmd(ftp_command_line,a:curbuf_line)

   if (a:curbuf_line < 0 ) 
    " remove enew buffer (quietly)
"    call Decho( "before cleanup bufnr(%)=".bufnr("%") )
    let tmpbuf= bufnr("%")
"    call Decho( "curbuf=".curbuf." tmpbuf=".tmpbuf)
    exe curbuf."b!"
    let &l:bh            = bhkeep
    exe tmpbuf."bw!"
   endif
"   call Decho( "end bufnr(%)=".bufnr("%") )
"   call Dret("NetrwFtpCmd")
endfun

	

" ---------------------------------------------------------------------
"  s:NetrwFtpPrepareCmd : prepare the common ftp commands
"  depending on the method 
"  	#2: ftp + .netrc
"  	#3: ftp + mip
"  	#9: sftp 
"  the general ftp commons are added to the command list in the current
"  buffer, and the command line to use is returned
"
"  netrw_ftpextracmd (if set) is added
"
"  The following setttings apply to the use of the IBM FTP server for
"  access to HFS(unixlike) filesystem and to MVS dataset-filesystem.
"
"  ftp command:
"  -i       : turns off interactive prompting from ftp
"  -n  unix : DON'T use <.netrc>, even though it exists
"  -n  win32: quit being obnoxious about password
fun s:NetrwFtpPrepareCmd( netrw_method, path  )
"   call Dfunc("NetrwFtpPrepareCmd(netrw_method<".a:netrw_method."> path=<".a:path.">")
    if a:netrw_method == 2  
"    	call Decho("via ftp+.netrc (method #2)")
    elseif a:netrw_method == 3  
"    	call Decho("via ftp+mipf (method #3)")
    endif

   if a:netrw_method == 2  " #2 ftp +netrc : specify machine and port on commandline
    let ftp_command_line=s:netrw_ftp_cmd
    let ftp_command_line = ftp_command_line ." -i ".shellescape(g:netrw_machine,1)
    if exists("g:netrw_port") && g:netrw_port != ""
     let ftp_command_line = ftp_command_line ." ".shellescape(g:netrw_port,1)
    endif
    let x= substitute( g:netrw_ftp_options, "-n", "", "")
    let x= substitute( x, "-i", "", "")
    let ftp_command_line = ftp_command_line . " ".x
   elseif a:netrw_method == 3 " # 3 ftp +mip : use open command with machine and or password
	    		      "                 and user/password commands
    let ftp_command_line=s:netrw_ftp_cmd
    if exists("g:netrw_port") && g:netrw_port != ""
    keepj put ='open '.g:netrw_machine.' '.g:netrw_port
    else
    ...
    let ftp_command_line = ftp_command_line . " ".g:netrw_ftp_options
   elseif a:netrw_method == 9 " # 3 sftp 
    let ftp_command_line=g:netrw_sftp_cmd
    if exists("g:netrw_port") && g:netrw_port != ""
     let ftp_command_line = ftp_command_line ." -P ".shellescape(g:netrw_port,1)
    endif
    let ftp_command_line = ftp_command_line ." ".shellescape(g:netrw_machine,1)
   endif

   if a:netrw_method == 2 || a:netrw_method ==3  " for plain old ftp
     "mode
     keepj put = g:netrw_ftpmode 

     if (g:netrw_ftpmode == 'ascii' )
       if exists("g:netrw_ftpextracmd")
        keepj put =g:netrw_ftpextracmd
       endif
       if ( g:netrw_ftp_enc_toggle_extracmd != '' ) 
          keepj put =g:netrw_ftp_enc_toggle_extracmd
       endif
     endif    
   endif

"   call Dret("NetrwFtpPrepareCmd ftp_command_line=".ftp_command_line)
   return ftp_command_line
endfun

" ---------------------------------------------------------------------
"  s:NetrwFtpExecuteCmd : execute the ftp command

fun s:NetrwFtpExecuteCmd( ftp_command_line, curbuf_line)
"   call Dfunc("NetrwFtpExecuteCmd(ftp_command_line<".a:ftp_command_line.">curbuf_line<".a:curbuf_line.">")
   ...
   let i=1
   while i <= lastline 
"	 Decho("Ftp-input[". i ."]=".getline(i))
	 if ( a:curbuf_line < 0 || i >= a:curbuf_line )
      	  call add ( error_msgs, "input[". i ."]=".getline(i))
	 endif
  	 let i = i + 1	
   endwhile
"   call Decho("exe ".s:netrw_silentxfer.",$!".s:netrw_ftp_cmd." -i ".shellescape(g:netrw_machine,1)." ".shellescape(g:netrw_port,1))

   if (a:curbuf_line < 0) 
	   let range="%!"
   else
	   let range=a:curbuf_line.",$!"
   endif
"   call Decho("executing: ".range.a:ftp_command_line)
   exe s:netrw_silentxfer." ".range.a:ftp_command_line

   let lastline = line("$")
   let i=1
   while i <= lastline 
"	 Decho("Ftp-output[". i ."]=".getline(i))
	 if ( a:curbuf_line < 0 || i >= a:curbuf_line )
      	  call add ( error_msgs, "output[". i ."]=".getline(i))
	 endif
  	 let i = i + 1	
   	 if getline(i) =~ g:netrw_sftp_errors  
		let has_error=1
"		Decho( "has_error on sftp: ". getline(i) )
	 endif
   endwhile

   "remove carriagereturn characters
   exe 'sil! keepj 1,$s@\r@@e'

   "if lcd command used, avoid error messages for its output
   exe "sil! keepalt keepj g/".g:netrw_ftp_lcd_msg."/keepj d"
   call histdel("/",-1)

   if ( a:curbuf_line < 0 )
    "If the result of the ftp operation isn't blank, show an error message (tnx to Doug Claar)
    if getline(1) !~ g:netrw_ignorebanner 
	let has_error=1
"	Decho( "has_error on first output line: ". getline(1) )
      	call add ( error_msgs, "ERROR: " .getline(1))
    endif
   endif
   ...
   if !exists("g:netrw_quiet") 
    if has_error == 1 
"      call Decho( "in error winnr()=".winnr(). " bufnr(%)=".bufnr("%") )
      let mybuf=bufnr("%")
      let mywin=winnr()
      let debugkeep= &debug
      setl debug=msg
      call netrw#ErrorMsgs(s:ERROR,error_msgs,15)
      let &debug= debugkeep
      let mod=1
      redraw!
      call inputsave()|call input("Press <cr> to continue")|call inputrestore()
      let errbuf=bufnr("%")
      let errwin=winnr()
"      call Decho( "after error winnr()=".winnr(). " errwin=".errwin. " bufnr(%)=".bufnr("%") . " mybuf=".mybuf )

      " make sure to proceed in same buffer and same window
      exe mywin."wincmd w"
      if mybuf != errbuf
     	exe mybuf."b!"
      endif
...
"   call Dret("NetrwFtpExecuteCmd return")
endfun
...
"      call Decho("subfrom<".subfrom."> subto<".subto."> newname<".newname.">")
    ...
    if newname != "" && newname != oldname
     if exists("w:netrw_method") && (w:netrw_method == 2 || w:netrw_method == 3 || w:netrw_method == 9 )
      let ftpcmds = []
      call add ( ftpcmds, 'cd "'.a:path.'"' )
      call add ( ftpcmds, 'rename "'.oldname.'" "'.newname.'"')
      call s:NetrwFtpCmd( -1, ftpcmds )
     else
...
"  call Decho("(s:LocalListing) g:netrw_banner=".g:netrw_banner.": banner ".(g:netrw_banner? "enabled" : "suppressed").": (line($)=".line("$")." byte2line(1)=".byte2line(1)." bannercnt=".w:netrw_bannercnt.")")
  ...
  let filelist = []
"
  "let filelist   = glob(s:ComposePath(dirname,"*",'r'),0,1)
 "
"  call Decho("call Myglob")
  let myfilelist   = s:Myglob(s:ComposePath(dirname,"*",'r'))
  for hh in split (myfilelist)
"    call Decho("HH: push filename<".hh.">")
    if ( hh == "Tab" ) 
"      call Decho("HH: skip Tab" )
    else
      call add(filelist,hh);
    endif
  endfor

  for filename in filelist
"    call Decho("HH1a: filename<".filename.">")
  endfor

  "let filelist   = filelist + glob(s:ComposePath(dirname,".*",'r'),0,1)
  "
  for filename in filelist
"    call Decho("HH1b: filename<".filename.">")
  endfor
  let myfilelist  = s:Myglob(s:ComposePath(dirname,".*",'r'))
  for filename in filelist
"    call Decho("HH1c: filename<".filename.">")
  endfor
  for hh in split (myfilelist)
"    call Decho("HH2: filename<".hh.">")
    call add(filelist,hh);
  endfor
  for filename in filelist
"    call Decho("HH1d: filename<".filename.">")
  endfor
...
"   call Decho("(LocalListing) forcibly including on \"..\"")
   let filelist= filelist+[s:ComposePath(b:netrw_curdir,"../",'d')]
"   call Decho("(LocalListing) filelist=".string(filelist))
  endif
...
"      call Decho("subfrom<".subfrom."> subto<".subto."> newname<".newname.">")
   ...
   while ctr <= a:lastline
    ...
    keepj norm! 0
    let oldname= s:ComposePath(a:path,curword,'fd')
"   call Decho("oldname<".oldname.">")
...
"  call Dfunc("s:NetrwLocalRmFile(path<".a:path."> fname<".a:fname."> all=".a:all)
  ...
  let ok = ""
  keepj norm! 0
  let rmfile= s:ComposePath(a:path,a:fname,'f')
...
fun! s:ComposePath(base,subdir,type)
"  call Dfunc("s:ComposePath(base<".a:base."> subdir<".a:subdir."> type<".a:type.">)")
"   call Decho("HH ComposePath")

  if  s:netrw_ftpservertype == s:FTPSERVERTYPE_IBM_FTP_MVS 
"   call Decho("HH ComposePath is MVS")
   if a:subdir == "./"
    let ret = a:base
   else
     if ( a:type == 'f' ) 
       let ret = substitute( a:base, "'$", "(".a:subdir.")'", "" )
     elseif ( a:type == 'd' ) 
       let ret = substitute( a:base, "'.*'", a:subdir , "" )
     else
       let ret = substitute( a:base, "'.*'", "'NOTSUPP'", "" )
     endif
   endif
...
fun! s:Myglob(path)
"  call Dfunc("s:Myglob(path<".a:path.">)")
  let myfilelist = glob(a:path)
"   for filename in split (myfilelist)
"   call Decho(":Myglob1 for filename in myfilelist: filename<".filename.">")
"  endfor
"  call Decho("s:Myglob2 myfilelist=".string(myfilelist))
"  call Decho("s:Myglob3 myfilelist=".myfilelist)
"  call Dret("s:Myglob")
  return myfilelist
endfun
...
"    	call Decho("HHH" . s:netrw_ftpservertype ) 
...
"  call Dfunc("s:NetrwInsureWinVars() win#".winnr() . " win($)#". winnr("$") )
  if !exists("w:netrw_liststyle")
   let curbuf = bufnr("%")
   let curwin = winnr()
   let iwin   = 1
...
"  call Dret("s:NetrwInsureWinVars win#".winnr(). " win($)#". winnr("$") )
endfun
